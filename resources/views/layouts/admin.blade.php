...
<body class="flex">
    ...
    <aside class="w-72 bg-slate-900 h-screen fixed text-white p-6 overflow-y-auto">
        ...
        <nav class="space-y-2">
            ...
            <!-- Animated Dropdown for Manage Trains -->
            <div x-data="{ open: {{ request()->routeIs('admin.schedules') || request()->routeIs('admin.fleet') ? 'true' : 'false' }} }" class="pt-2">
                ...
                <div x-show="open" x-collapse x-cloak>
                    <div class="mt-1 space-y-1 bg-slate-950/50 rounded-xl p-2 border border-slate-800/50">
                        <a href="{{ route('admin.schedules') }}" class="block p-3 pl-11 rounded-lg text-sm transition {{ request()->routeIs('admin.schedules') ? 'text-white font-bold bg-slate-800/80' : 'text-slate-500 hover:bg-slate-800/50 hover:text-slate-300' }}">
                            Schedules
                        </a>
                        <a href="{{ route('admin.fleet') }}" class="block p-3 pl-11 rounded-lg text-sm transition {{ request()->routeIs('admin.fleet') ? 'text-white font-bold bg-slate-800/80' : 'text-slate-500 hover:bg-slate-800/50 hover:text-slate-300' }}">
                            Train Fleet
                        </a>
                    </div>
                </div>
            ...
            <div x-data="{ open: {{ request()->routeIs('admin.logs') ? 'true' : 'false' }} }" class="pt-2">
                ...
                <div x-show="open" x-collapse x-cloak>
                    <div class="mt-1 space-y-1 bg-slate-950/50 rounded-xl p-2 border border-slate-800/50">
                        <a href="{{ route('admin.logs') }}" class="block p-3 pl-11 rounded-lg text-sm transition {{ request()->routeIs('admin.logs') ? 'text-white font-bold bg-slate-800/80' : 'text-slate-500 hover:bg-slate-800/50 hover:text-slate-300' }}">
                            Admin Logs
                        </a>
                    </div>
                </div>
        ...
        <header class="flex justify-between items-center mb-10">
            ...
            <div class="flex items-center space-x-4">
                <div class="text-right">
                    ...
                    <p class="text-xs text-slate-400">admin@swift.com</p>
                </div>
                <img src="https://ui-avatars.com/api/?name=Admin&background=E31E24&color=fff" class="w-12 h-12 rounded-full border-2 border-white shadow-lg">
            </div>
        </header>
